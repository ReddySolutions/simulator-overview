...
class TestFlagOn:
    ...
    async def test_self_critique_off_does_not_call_surgical_review(self) -> None:
        """QA_ENABLE_LLM_CRITIC=on + ENABLE_SELF_CRITIQUE=off -> one call only."""
        project = _project(
            _tree(
                _screen(
                    "s1",
        ...
        client = _make_client([v1_text])

        result = await validate(project, client=client)

        assert len(result.findings) == 1
        assert "c1" in result.findings[0].message
        assert client.messages.create.await_count == 1
...
class TestSelfCritiqueOn:
    ...
    async def test_surgical_review_v2_differs_from_v1(self) -> None:
        """Flag on: reviewer's edits change the emitted finding message."""
        project = _project(
            _tree(
                _screen(
                    "s1",
        ...
        v1_text = json.dumps(
            {
                "unsupported_claims": [
                    {"claim": "Original claim", "reason": "Original reason"}
                ]
            }
        )
        edits_text = json.dumps(
            {
                "edits": [
                    {
                        "path": "unsupported_claims.0.claim",
                        "operation": "replace",
        ...
        assert len(result.findings) == 1
        assert "Reviewed claim" in result.findings[0].message
        assert "Original claim" not in result.findings[0].message
        # Two HTTP calls: v1 + surgical_review
        assert client.messages.create.await_count == 2
