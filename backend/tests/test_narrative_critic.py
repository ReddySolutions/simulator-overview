...
class TestFlagOn:
    ...
    async def test_self_critique_off_does_not_call_surgical_review(self) -> None:
        """QA flag on, self-critique off -> one call per screen."""
        response_payload = {
            "unsupported": [
                {
                    "screen_id": "s1",
        ...
        assert len(result) == 1
        assert "Original claim" in result[0].description
        # Only the v1 call -- surgical_review must NOT be invoked
...
class TestSelfCritiqueOn:
    ...
    async def test_surgical_review_v2_differs_from_v1(self) -> None:
        """Flag on: reviewer edits the claim; emitted gap reflects v2."""
        v1_payload = {
            "unsupported": [
                {
                    "screen_id": "s1",
        ...
        edits_payload = {
            "edits": [
                {
                    "path": "unsupported.0.claim",
                    "operation": "replace",
                    "new_value": "Reviewed claim",
                },
            ]
        }
        client = _make_client(
            [json.dumps(v1_payload), json.dumps(edits_payload)]
        )
        ...
        assert len(result) == 1
        assert "Reviewed claim" in result[0].description
        assert "Original claim" not in result[0].description
        # v1 + surgical_review per screen = 2 calls
        assert client.messages.create.await_count == 2
